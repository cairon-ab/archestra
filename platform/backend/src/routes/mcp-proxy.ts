...
/**
 * MCP Proxy routes for frontend MCP Apps (AppRenderer)
 * Provides session-based auth access to MCP Gateway endpoints.
 *
 * Unlike the MCP Gateway (/v1/mcp/:profileId) which requires a Bearer token,
 * this proxy uses the user's existing web session for authentication.
 * This allows the frontend to proxy JSON-RPC requests on behalf of the user
 * without requiring a separate MCP token.
 */
  ...
  fastify.post(
    "/api/mcp/:agentId",
    {
      schema: {
        ...
        tags: ["mcp-proxy"],
        description:
          "Proxy MCP Gateway requests with session auth for MCP Apps",
        params: z.object({
    ...
    async (request, reply) => {
      ...
      let hijacked = false;
      let server: Awaited<ReturnType<typeof createAgentServer>>["server"] | undefined;
...
type McpServer = Awaited<ReturnType<typeof createAgentServer>>["server"];
